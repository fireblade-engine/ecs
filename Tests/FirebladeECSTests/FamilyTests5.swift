...
@Suite struct FamilyTests5 {
    ...
    @Test func memberCreation() {
        ...
        let entity = family.createMember(with: Comp1(1), Comp2(2), Comp3(3), Comp4(4), Comp5(5))
    ...
    @Test func componentIteration() {
        ...
        let count = 100
        for i in 0..<count {
            family.createMember(with: Comp1(i), Comp2(i * 2), Comp3(i * 3), Comp4(i * 4), Comp5(i * 5))
        }
        
        #expect(family.count == count)
        
        var i = 0
        family.forEach { (c1: Comp1, c2: Comp2, c3: Comp3, c4: Comp4, c5: Comp5) in
            #expect(c1.value == i)
            #expect(c2.value == i * 2)
            #expect(c3.value == i * 3)
            #expect(c4.value == i * 4)
